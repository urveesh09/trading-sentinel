...
count. Paths are relative to the captures directory.
...
1. Run the staging-only probe for each branch. ISO 8601
   ...
   python -m tools.j2_cas_probe.py        --symbols RELIANCE        --observation-at 2026-09-14T15:17:00        --output docs/j2_captures/2026-09-14/RELIANCE_15_17.json

   # CAS_ORDER_ENTRY example (different time, same day)
   python -m tools.j2_cas_probe.py        --symbols RELIANCE        --observation-at 2026-09-14T15:22:00        --output docs/j2_captures/2026-09-14/RELIANCE_15_22.json
   ```
2. Validate each capture:
   ```bash
   python -m tools.j2_capture_review.py        docs/j2_captures/2026-09-14/RELIANCE_15_17.json
   ```
