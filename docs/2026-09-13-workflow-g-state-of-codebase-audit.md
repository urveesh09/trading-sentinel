...
Status: G inventory documented at commits `16fd6af` (audit/bridge docs), `8c15ad6` (range-reversion constant), `877fae6` (promotion bridge persistence + state machine), `b2efc44` (trailing-stop exit profile), and the legacy-run-id shim closure (this commit). G is now `IMPLEMENTING — DOCUMENTED_SHIM` with five of six gaps closed. The remaining gap is #6 (session_phase forward-compat on shadow trial rows; deferred to J's session-phase design). Future user direction needed on whether to (a) wait for J's design before closing #6, (b) ship a placeholder `session_phase = NULL` column now and let J refine, or (c) defer G implementation entirely until F + D close. Verified audit commit `16fd6af`; constant-change commit `8c15ad6`; bridge commit `877fae6`; trailing-stop commit `b2efc44`; shim-closure commit SHA will be added when committed.
