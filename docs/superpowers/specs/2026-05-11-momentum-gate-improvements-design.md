...
On 2026-05-11 at 11:45 IST, the momentum screener fired a signal for **TECHM** in a `BEAR_RS_ONLY` regime. The trade was executed at Rs1,468.70. The stock peaked at Rs1,471.40 (a +2.70 point / 0.20R move) before bleeding for 3 hours and auto-squaring at ~Rs1,457.50 -- a loss of ~0.83R.
...
**Would TECHM have been caught?** At 1.5R, target = 1,468.7 + (1.5 x 13.5) = **1,489.0**. Still unreachable (peak was 1,471.4). BUT: the ATR gate (Gate A) would have caught it already. This gate reduces *expected loss* on the trades that slip through, by setting a tighter, more achievable target in weak markets.
...
MR2  Target = r_target x R (regime-adjusted)        [modified -- Gate D]
...
- [OK] No TBDs or placeholders
- [OK] No contradictions between sections
- [OK] Gate thresholds calibrated to avoid over-filtering (1.75x not 2.0x, 0.65 not 0.75, 0.85 buffer not 1.0)
- [OK] All data sources verified against actual code -- no new API calls required
- [OK] `calc_atr()` already implemented and tested in engine.py
- [OK] `df_daily` already fetched in `run_momentum_screener()` -- no extra fetch
- [OK] Inviolable rules respected: engine functions remain pure (time logic stays in main.py via `vol_surge_threshold` param)
