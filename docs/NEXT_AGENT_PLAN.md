...
|| G — strategy basket and entry/exit intelligence | IMPLEMENTING (RANGE_REVERSION_V1 constant landed; gap #1 of 6 closed) | `docs/2026-09-13-workflow-g-state-of-codebase-audit.md` for shipped-substrate census, hypothesis coverage, no-execution invariant and coupling; `docs/2026-09-13-workflow-g-promotion-bridge.md` for the signed-state machine that any future live promotion must obey; `python-engine/tests/test_range_reversion_profile.py` for the 5-test focused suite; whole-engine suite is 2,574 passed/4 skipped/23 warnings at 122.52s (+29 tests vs. previous 2,545 baseline, no regression to the previously closed 17 baseline failures) | Five remaining gaps: promotion bridge refusal/risk-budget surfaces, comparison protocol, `default-v1` shim doc, `session_phase` forward-compat (deferred to J), optional 3rd exit profile. Residual: dispatcher still routes RANGE_REVERSION_V1 through completed-bar-confirmation fallback (not faithful range semantics); a dedicated range simulator is a separate future slice. Live promotion still requires F + D closure |
