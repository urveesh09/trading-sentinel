...
| F — trading losses and accounting truth | IMPLEMENTING (F1 inventory + F2 affordability guard + F3 open MTM + F4 discrepancy-ID framework landed; F5-F6 remaining) | `docs/2026-09-13-workflow-f-state-of-codebase-audit.md` sections 7-9: standalone `affordability.py` shipped (margin=1.5, paper_pnl_ratio=2.0, six named verdicts); standalone `mark_to_market.py` shipped (FRESH/STALE/UNAVAILABLE buckets, multi-leg conservative semantics); standalone `discrepancies.py` shipped (nine `DiscrepancyCategory` values, forward-only state machine, append-only DB triggers, idempotent recording on `(category, evidence_key)`, two bridge functions for broker_statement_report and reconciliation_evidence_report, plus a combined `record_current_state`). `main.py:run_penny_hourly_report` is wired to call `mark_open_positions` with a batched `kite.get_quote(tokens)` lookup and the silent-false-loss bug at line 1599 is closed. 34 + 8 + 43 + 42 tests passed; whole-engine rerun 2,836 passed/4 skipped/23 warnings in 126.59s (+127 vs. previous 2,709 baseline; +42 vs. F3 baseline of 2,794). No regression to the previously closed baseline failures; no new warnings | F5 broker statement automation skeleton (CLI + route, no scheduler) -- the natural wiring site for `record_current_state`; F6 capital policy + user-supplied loss tolerance -- each gated on the previous slice |
