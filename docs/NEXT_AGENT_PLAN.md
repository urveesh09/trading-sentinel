...
| F — trading losses and accounting truth | IMPLEMENTING (inventory corrected; genuine acceptance unproven) | Independent source audit corrected owning schemas/PKs, multiple ledger writers, source/default scope and unproven A1–A5 closures in `2026-09-13-workflow-f-state-of-codebase-audit.md`. Equity effective date restored to unknown; verification date/rates/options/immutable snapshots retained. Fresh owner-schema and broker multi-entry tests added; eight-file acceptance150 passed with warnings fatal; ten-file154 passed with one existing Starlette deprecation | Actual ORB loss reconstruction, warning/source/statement evidence, broker-to-local matching, affordability/MTM/funding/partial/fill audits and externally reconciled capital policy with user risk input. No tariff effectiveness or historical discrepancy closure inferred from tests |
