...
|| G — strategy basket and entry/exit intelligence | INVENTORY_ONLY | `docs/2026-09-13-workflow-g-state-of-codebase-audit.md` for shipped-substrate census, hypothesis coverage, no-execution invariant and coupling; `docs/2026-09-13-workflow-g-promotion-bridge.md` for the signed-state machine that any future live promotion must obey | Six gaps from audit §6 closed (notably `RANGE_REVERSION_V1` profile and a third exit), user-declared loss tolerance, then `APPROVED_WITH_BUDGET` per bridge §8; live promotion still requires F + D closure |
