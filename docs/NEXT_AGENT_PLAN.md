...
| F — trading losses and accounting truth | IMPLEMENTING (F1 inventory + reconciliation-warning map closed of the six F slices) | `docs/2026-09-13-workflow-f-state-of-codebase-audit.md` catalogs every F-relevant table, write path, read path, source-enum invariant, and historical-warning discrepancy map; `EQUITY_INTRADAY_EFFECTIVE_DATE` pinned to `"2026-08-10"` matching `EQUITY_INTRADAY_VERIFIED_AS_OF`. Focused 8-test suite `python-engine/tests/test_F_phase1_inventory.py` proves the cost-schedule decision and the F4 clean slate. Whole-engine reruns 2,630 passed (no regressions) | F2 paper-vs-live affordability guard, F3 mark-to-market, F4 discrepancy-ID framework, F5 broker automation, F6 capital policy per the phase plan in the F audit |
