...
| H — scheduling / provider efficiency / dashboard | IMPLEMENTING (H1 scheduler coroutine warning closed and PROD-ready; H2 priority-tier breakdown closed; H3 intraday-cache caller/key/window diagnostic closed; remaining H5 deferred) | H1 — `python-engine/scheduler_setup.py`: ``run_penny_hourly_report_safe`` wrapper + cron re-registered with ``max_instances=1, coalesce=True, misfire_grace_time=600``; ALL_CLOSURES extended. H2 — `python-engine/scheduler_telemetry.py`: `JOB_TIER_MAP` + `TIER_ORDER` + `_tier_for` + `_aggregate_by_tier` + `by_tier` key in `scheduler_timing_report`; `operational_coverage.py` extended with `scheduler_tier:{tier}` entries. H3 — `python-engine/intraday_cache_diagnostic.py`: read-only diagnostic with `CACHED_CALLER_SITES`/`UNCACHED_CALLER_SITES` inventories, `cache_row_counts`/`cache_interval_breakdown`/`cache_freshness_window`/`audit_key_shape` queries, `run_diagnostic` + CLI; reuses `reconciliation_cli._write_output_atomic`. The diagnostic surfaces three failure modes the audit doc conflated: zero-callers / freshness-gated-misses / cold-cache; audits the PRIMARY KEY against §12 cross-account/cross-token invariant. 7 + 22 + 26 = 55 new H-series tests pass. whole-engine 2978 passed/4 skipped/39 warnings in 130.89s (+55 vs previous 2,923 — corrected from 2,952 baseline before H3). No regression to the previously closed baseline failures; no new warnings | H5 dashboard readiness reasons (H4 cache-add deferred per §12 explicit ordering pending operator sign-off on cache key shape + freshness budget) |
