...
| H — scheduling / provider efficiency / dashboard | IMPLEMENTING (H1 scheduler coroutine warning closed and PROD-ready; remaining H2/H3/H5 deferred) | `python-engine/scheduler_setup.py`: ``run_penny_hourly_report_safe`` wrapper added inside ``register_penny_scheduler_jobs`` with first-line breadcrumb + calendar gate (inside try/except) + try/except wrapping the body; cron registration switched to the wrapper with ``max_instances=1, coalesce=True, misfire_grace_time=600``. ``python-engine/tests/test_scheduler_closures_invoke.py``: ``run_penny_hourly_report_safe`` added to ``ALL_CLOSURES`` (was the only penny subsystem job not in the parametrised roster). ``python-engine/tests/test_scheduler_h1_coroutine_guards.py``: 7 new tests (closure registration, coroutine leak, breadcrumb fires on success, standard exception catching, create_task-await cycle, F3 import failure resistance). ``tests/main_surface_golden.json``: deliberately regenerated via ``TS_UPDATE_GOLDEN=1``; diff is exactly one line (``func: run_penny_hourly_report`` -> ``func: run_penny_hourly_report_safe``), no other drift. 7 new H1 tests passed; closure-invoke went from 24 -> 25; whole-engine rerun 2,930 passed/4 skipped/39 warnings in 129.02s (+8 vs. previous 2,922 baseline). No regression to the previously closed baseline failures; no new warnings. Production smoke test (substrate failure injected into ``is_trading_day``) confirmed the wrapper logs the breadcrumb, logs the failure with full traceback, and does NOT raise | H2 scheduler timing priority-tier breakdown, H3 intraday-cache caller/key/window diagnostic, H5 dashboard readiness reasons (H4 cache-add deferred per §12 explicit ordering) |
| I — optional AI and news | IMPLEMENTING (annotation validity TESTED_DEV) | `docs/2026-09-13-optional-ai-validity-plan.md`: reproduced stale READY/CACHED after original expiry; nested input snapshot and original/shortened deadline/cache TTL/exact-completion regressions. Isolated network-disabled agent suite 98 passed with warnings fatal | Complete model/prompt/source provenance, sourced-news publication/event timestamps, bounded provider timeout and annotation usefulness acceptance; optional per plan §15 |
| J — CAS and market-session correctness | NOT_STARTED by this slice | See workstream J audit recommendation; G-side seam shipped at `proactive_intelligence.py::stamp_session_phase` waiting for the source inventory | Real classifier lands J-side; G placeholder is forward-compatible |
