...
| C — replay fidelity/review integration | IMPLEMENTING | Real evaluator/archive multi-session fixture reaches one costed close and one unresolved entry; conflicting same-receipt packets are rejected; verified full-policy reports feed held-out aggregation | Finish source scope/roll context, delayed/partial execution boundaries, immutable cost stress and ordered-drawdown review evidence |
