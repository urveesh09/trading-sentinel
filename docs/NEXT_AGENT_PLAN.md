...
| H — scheduling / provider efficiency / dashboard | IMPLEMENTING (H1 scheduler coroutine warning closed and PROD-ready; H2 priority-tier breakdown closed; H3 intraday-cache caller/key/window diagnostic closed `fead40c`; H4 cache-add for ``get_intraday`` closed with three operator-tunable knobs honouring §12 four explicit semantics; H4.B by-token cache deferred pending operator sign-off via proposal doc `docs/2026-09-13-h4-by-token-cache-proposal.md`; H5 dashboard readiness reasons deferred) | H1 — `python-engine/scheduler_setup.py`: ``run_penny_hourly_report_safe`` wrapper + cron re-registered with ``max_instances=1, coalesce=True, misfire_grace_time=600``; ALL_CLOSURES extended. H2 — `python-engine/scheduler_telemetry.py`: `JOB_TIER_MAP` + `TIER_ORDER` + `_tier_for` + `_aggregate_by_tier` + `by_tier` key in `scheduler_timing_report`; `operational_coverage.py` extended with `scheduler_tier:{tier}` entries. H3 — `python-engine/intraday_cache_diagnostic.py`: read-only diagnostic with `CACHED_CALLER_SITES`/`UNCACHED_CALLER_SITES` inventories, `cache_row_counts`/`cache_interval_breakdown`/`cache_freshness_window`/`audit_key_shape` queries, `run_diagnostic` + CLI; reuses `reconciliation_cli._write_output_atomic`. H4 — `python-engine/kite_client.py:454-575`: explicit §12 four-semantics enforcement in `get_intraday` HIT path (instrument + interval + completed-bar cutoff + freshness); three new operator-tunable knobs in `python-engine/config.py`: `INTRADAY_CACHE_FRESHNESS_SECONDS=0` (leniency budget on the existing freshness gate), `INTRADAY_CACHE_INCLUDE_FORMING=False` (§12 completed-bar cutoff, excludes forming candles from returned set), `INTRADAY_CACHE_MIN_CANDLES=4` (existing VWAP floor made explicit). Defaults preserve pre-H4 behaviour exactly. Docstring at `kite_client.py:454` rewritten to reflect the four explicit semantics. 7 + 22 + 26 + 13 = 68 new H-series tests pass. whole-engine 2,991 passed/4 skipped/39 warnings (~+13 vs previous 2,978 after H3). No regression to the previously closed baseline failures; no new warnings. | H5 dashboard readiness reasons |
