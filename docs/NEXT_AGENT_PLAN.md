...
| J — CAS and market-session correctness | IMPLEMENTING (J.1 classifier closed `d82258f`/`63a98ab`; J.2.1 eligibility list closed `ba117fb`; J.2.2 broker-behaviour probe closed `f6e07da`; J.2 docs sweep in this slice) | `docs/2026-09-13-workflow-j-deep-research.md`: verified NSE/BSE facts (CAS Phase 1 effective 2026-01-19 via NSE/CMTR/72394; index-futures CAS-aligned band effective 2026-09-07 via NSE/CMTR/76170; BSE CAS derivatives 2026-08-03). `python-engine/market_calendar.py`: central session constants block (MARKET_OPEN_TIME 09:15 / MARKET_CLOSE_TIME 15:30 cash / DERIVATIVES_CLOSE_TIME 15:40 / CAS_OPEN_TIME 15:15 / CAS_REFERENCE_PRICE_END 15:20 / CAS_ORDER_ENTRY_END 15:25 / CAS_LIMIT_ENTRY_ONLY_END 15:30 / CAS_MATCHING_END 15:35 / CAS_POST_CLOSE_END 16:00) + `classify_session_phase(observation_at, *, symbol, is_derivative)` total classifier returning 10 bounded phases + `is_cas_eligible(symbol)` consults `settings.CAS_PHASE1_FNO_UNDERLYINGS` (lazy import, case+whitespace normalised, ``functools.lru_cache``-memoised). `python-engine/config.py`: `CAS_PHASE1_FNO_UNDERLYINGS: str = ""` (CSV wire format). `python-engine/tools/j2_cas_probe.py` NEW: staging CLI `--dry-run` / `--now` / `--observation-at` / `--output` / exit codes 0/1/2. `docs/2026-09-13-j2-broker-behaviour-probe.md` NEW: six-point operator review checklist. 41 J.1 + 6 J.2 tests pass; J-slice five-file suite 106 pass; F/G + J critical paths 241 pass warnings fatal. J.1 `stamp_session_phase` (G forward-compat seam) UNCHANGED. | Broker-behaviour verification pending in staging (cannot run in Dev without a broker). `fno_chain.py::EXPIRY_CUTOFF_HOUR/_MIN = 15, 30` and `hedge_strategies.py::_EXPIRY_CUTOFF = time(15, 30)` preserved (per plan §14: no deadline change without operator sign-off). `market-hours.js` (Container B) parity deferred. Holiday reconciliation Python↔Node deferred. |
