...
|| G — strategy basket and entry/exit intelligence | IMPLEMENTING (gaps #1, #2, #5 and #6 closed of 6) | `docs/2026-09-13-workflow-g-state-of-codebase-audit.md` for shipped-substrate census, hypothesis coverage, no-execution invariant and coupling; `docs/2026-09-13-workflow-g-promotion-bridge.md` for the signed-state machine; `python-engine/tests/test_range_reversion_profile.py` (5 tests), `python-engine/tests/test_promotion_bridge.py` (30 tests), `python-engine/tests/test_trailing_stop_profile.py` (8 tests), and `python-engine/tests/test_legacy_run_id_shim.py` (9 tests); whole-engine suite is 2,621 passed/4 skipped/23 warnings at 122.35s (+76 tests vs. previous 2,545 baseline, no regression to the previously closed 17 baseline failures) | Two remaining gaps: pre-declared comparison protocol inheriting C's `intraday_spread_holdout` split rules; `session_phase` forward-compat on shadow trial rows (deferred to J). Residuals: (i) RANGE_REVERSION_V1 dispatcher still routes through completed-bar-confirmation fallback; (ii) no operator has yet signed a BridgeDecision (loss tolerance + INITIAL_BANKROLL cap pending user input); (iii) the `default-v1` shim is now codified (single source of truth via `LEGACY_DEFAULT_V1_RUN_ID`) but the migration path remains operator-driven. Live promotion still requires F + D closure |
