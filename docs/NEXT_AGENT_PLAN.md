...
| H — scheduling / provider efficiency / dashboard | IMPLEMENTING (zero-cache-hit diagnostic only, gated while C is in flight) | Earlier G session expanded H into a separate workstream; the current F1 slice is read-only and adds no H change | C-flight gating per session policy |
