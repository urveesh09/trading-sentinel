...
| H — scheduling / provider efficiency / dashboard | IMPLEMENTING (H1 scheduler coroutine warning closed and PROD-ready; H2 priority-tier breakdown closed; remaining H3/H5 deferred) | `python-engine/scheduler_telemetry.py`: `JOB_TIER_MAP` (every registered penny/fno/partner/system job_id classified into one of the four §12 priority tiers + a `system` meta-tier); `TIER_ORDER`; `_tier_for(job_id)` returning `"other"` for unrecognised ids; `_aggregate_by_tier(jobs)` computing per-tier p50/p95/max across the UNION of samples (NOT per-job medians) and per-stage `stage_durations` percentiles; `scheduler_timing_report` extended with a top-level `by_tier` key (existing `boot_id`/`events`/`jobs`/`inflight`/`note` keys remain byte-identical). A tier with zero jobs returns `None` (not `0`) for all numeric fields per §12 *"UI fixture covers unavailable and zero distinctly."* `python-engine/operational_coverage.py`: `scheduler_tier:{tier}` entry per tier so `/analytics/operational-coverage` surfaces tier-level coverage. 22 new H2 tests pass; existing 5 telemetry + 1 operational-coverage tests unchanged. Whole-engine 2952 passed/4 skipped/39 warnings in 131.89s (+22 vs previous 2,930). No regression to the previously closed baseline failures; no new warnings | H3 intraday-cache caller/key/window diagnostic, H5 dashboard readiness reasons (H4 cache-add deferred per §12 explicit ordering) |
