...
| J — CAS and market-session correctness | IMPLEMENTING (J.1 classifier closed `d82258f`/`63a98ab`; J.2.1 eligibility list closed `ba117fb`; J.2.2 broker-behaviour probe closed `f6e07da`; J.2 docs sweep closed `211fc58`; J.3.1 probe-quality + classifier `cas_eligible` kwarg closed `6019279`; J.3 capture-review tool + runbook + receipt-directory scaffold closed `9ab8ad1`; **J.4 wire `stamp_session_phase` to real classifier closed `cb309a8`**; **operator-supplied staging captures still pending**) | J.1: `python-engine/market_calendar.py` central constants + 10-phase classifier + `is_cas_eligible` + 41 tests. J.2.1: `CAS_PHASE1_FNO_UNDERLYINGS` env var (CSV) → `is_cas_eligible`. J.2.2: staging-only `tools/j2_cas_probe.py`. J.3.1: probe bumped to `SCHEMA_VERSION: 2` with inline JSON Schema; new flags `--schema-print` / `--eligibility-list` / `--require-eligible` / `--validate`; strict ISO 8601 parser (naive timestamps rejected); explicit `cas_eligible: bool | None = None` kwarg on `classify_session_phase` (default `None` preserves J.1 byte-identity, explicit values short-circuit the settings lookup). J.3: NEW `tools/j2_capture_review.py` (six-point checklist + opt-in OHLC-continuity cross-window check); NEW `docs/2026-09-13-j3-capture-protocol.md`; NEW `docs/j2_captures/README.md`. J.4: `python-engine/proactive_intelligence.py::stamp_session_phase` wired to `classify_session_phase` via lazy import. Signature gains three opt-in keyword-only kwargs (`symbol`, `is_derivative`, `cas_eligible`). `observation_at=None` still returns `"UNKNOWN"` (preserves `_ensure_shadow_run` manifest); a real datetime returns the bounded phase. NEW `tests/test_j4_stamp_session_phase.py`: 17 tests across 5 classes. 3225 / 4 skipped / 2 documented-isolation failures in the full repository suite (vs baseline 3208 / 4 / 1); +17 net (J.4 tests), 0 regressions. J-slice warning-fatal 332 pass; F/G + J critical paths 12-file 283 pass. | **Operator-supplied captures still pending in staging.** Without the receipt, J row cannot move from `IMPLEMENTING` to `TESTED_DEV`. The probe + review tool + classifier wiring are all Dev-verified; staging verification closes J. `fno_chain.py::EXPIRY_CUTOFF_HOUR/_MIN = 15, 30` and `hedge_strategies.py::_EXPIRY_CUTOFF = time(15, 30)` preserved unchanged. `market-hours.js` parity deferred. Holiday reconciliation Python↔Node deferred. |
