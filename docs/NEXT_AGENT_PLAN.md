...
| F — trading losses and accounting truth | IMPLEMENTING (F1 inventory + F2 affordability guard + F3 open MTM landed; F4-F6 remaining) | `docs/2026-09-13-workflow-f-state-of-codebase-audit.md` sections 7-8: standalone `affordability.py` shipped (margin=1.5, paper_pnl_ratio=2.0, six named verdicts) and standalone `mark_to_market.py` shipped (FRESH/STALE/UNAVAILABLE buckets, multi-leg conservative semantics). `main.py:run_penny_hourly_report` is wired to call `mark_open_positions` with a batched `kite.get_quote(tokens)` lookup and the silent-false-loss bug at line 1599 is closed (the previous expression silently returned `-entry_price * shares` because `positions` has no `current_price` column). 34 unit + 8 integration + 43 MTM tests passed; whole-engine rerun 2,794 passed/4 skipped/23 warnings in 125.65s (+85 vs. previous 2,709 baseline; +43 vs. F2 baseline of 2,751). No regression to the previously closed baseline failures; no new warnings. Affordability and MTM call-site scaffolds with `[AFFORDABILITY-SEAM 2026-09-13]` markers placed in `penny_edge_orchestrator.run_penny_edge_scan` and `fno_orchestrator.run_fno_tick` so live growth and F&O open MTM will plug into the producer when the operator enables them | F4 discrepancy-ID framework (tables + IDs), F5 broker statement automation skeleton (CLI + route, no scheduler), F6 capital policy + user-supplied loss tolerance — each gated on the previous slice |
