...
| F — trading losses and accounting truth | IMPLEMENTING (F1 inventory + F2 affordability guard + F3 open MTM + F4 discrepancy-ID framework + F5 broker statement automation skeleton landed; F6 remaining) | `docs/2026-09-13-workflow-f-state-of-codebase-audit.md` sections 7-10: standalone `affordability.py` shipped (margin=1.5, paper_pnl_ratio=2.0, six named verdicts); standalone `mark_to_market.py` shipped (FRESH/STALE/UNAVAILABLE buckets, multi-leg conservative semantics); standalone `discrepancies.py` shipped (nine `DiscrepancyCategory` values, forward-only state machine, append-only DB triggers, idempotent recording on `(category, evidence_key)`); standalone `reconciliation_cli.py` shipped (three subcommands: import-statement, run-report, list-discrepancies) plus two new routes `POST /reconciliation/import-statement` and `GET /reconciliation/discrepancies` in `routes_commands.py`. `main.py:run_penny_hourly_report` is wired to call `mark_open_positions`. `tests/main_surface_golden.json` was deliberately regenerated (2 routes added, no other drift) via `TS_UPDATE_GOLDEN=1`. 34 + 8 + 43 + 42 + 29 + 16 tests passed; whole-engine rerun 2,881 passed/4 skipped/39 warnings in 128.75s (+172 vs. previous 2,709 baseline; +45 vs. F4 baseline of 2,836). No regression to the previously closed baseline failures; +16 warnings are the existing `'app' shortcut` DeprecationWarning pattern from route tests, not a new warning category | F6 capital policy + user-supplied loss tolerance -- the final F slice |
