...
|| G — strategy basket and entry/exit intelligence | IMPLEMENTING (gaps #1 and #2 closed of 6) | `docs/2026-09-13-workflow-g-state-of-codebase-audit.md` for shipped-substrate census, hypothesis coverage, no-execution invariant and coupling; `docs/2026-09-13-workflow-g-promotion-bridge.md` for the signed-state machine; `python-engine/tests/test_range_reversion_profile.py` (5 tests) and `python-engine/tests/test_promotion_bridge.py` (30 tests); whole-engine suite is 2,604 passed/4 skipped/23 warnings at 123.04s (+59 tests vs. previous 2,545 baseline, no regression to the previously closed 17 baseline failures) | Four remaining gaps: pre-declared comparison protocol inheriting C's `intraday_spread_holdout` split rules, `default-v1` shim doc, `session_phase` forward-compat (deferred to J), optional 3rd exit profile. Residuals: (i) RANGE_REVERSION_V1 dispatcher still routes through completed-bar-confirmation fallback; (ii) no operator has yet signed a BridgeDecision (loss tolerance + INITIAL_BANKROLL cap pending user input). Live promotion still requires F + D closure |
