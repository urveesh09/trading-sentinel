...
| G — strategy basket and entry/exit intelligence | IMPLEMENTING (gaps #1, #2, #5 and #6 closed of 6) | `docs/2026-09-13-workflow-g-state-of-codebase-audit.md` for shipped-substrate census, hypothesis coverage, no-execution invariant and coupling; `docs/2026-09-13-workflow-g-promotion-bridge.md` for the signed-state machine; `python-engine/tests/test_range_reversion_profile.py` (5 tests), `python-engine/tests/test_promotion_bridge.py` (30 tests), `python-engine/tests/test_trailing_stop_profile.py` (8 tests), `python-engine/tests/test_legacy_run_id_shim.py` (9 tests), and `python-engine/tests/test_session_phase_placeholder.py` (9 tests); whole-engine suite is 2,630 passed/4 skipped/23 warnings at 122.96s (+85 tests vs. previous 2,545 baseline, no regression to the previously closed 17 baseline failures) | Gap #4 (pre-declared comparison protocol inheriting C's `intraday_spread_holdout` split rules) intentionally deferred while C is in flight. Residuals: (i) RANGE_REVERSION_V1 dispatcher still routes through completed-bar-confirmation fallback; (ii) no operator has yet signed a BridgeDecision (loss tolerance + INITIAL_BANKROLL cap pending user input); (iii) the `default-v1` shim migration path remains operator-driven; (iv) the session-phase schema column on `proactive_shadow_research_trials` is J-owned. Live promotion still requires F + D closure |
