...
| F — trading losses and accounting truth | IMPLEMENTING (F1 inventory + F2 affordability guard landed; F3-F6 remaining) | `docs/2026-09-13-workflow-f-state-of-codebase-audit.md` section 7: standalone `affordability.py` shipped with sync `evaluate_paper_to_live_affordability`, sync `assert_live_affordable_from_paper`, async `assert_live_entry_safety`, dataclasses `AffordabilityThresholds` / `AffordabilityEvaluation` / `AffordabilityVerdict`, and `AffordabilityRefusal`. Default thresholds derive from the promotion-bridge contract (margin=1.5, paper_pnl_ratio=2.0). 34 unit + 8 integration tests passed; whole-engine rerun 2,751 passed/4 skipped/23 warnings in 125.66s (+42 vs. previous 2,709 baseline, no regression to the previously closed baseline failures, no new warnings). Integrator call-site scaffolds with `[AFFORDABILITY-SEAM 2026-09-13]` markers placed in `penny_edge_orchestrator.run_penny_edge_scan` and `fno_orchestrator.run_fno_tick` so live growth will plug into the guard when the operator enables it | F3 open mark-to-market, F4 discrepancy-ID framework (tables + IDs), F5 broker statement automation skeleton (CLI + route, no scheduler), F6 capital policy + user-supplied loss tolerance — each gated on the previous slice |
