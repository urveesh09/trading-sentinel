...
Block deals on NSE occur during a **15-minute window** (09:15–09:30 or 14:45–15:00) where large orders (≥₹5 crore per order) are matched at a single price:

- **Minimum order size:** ₹5 crore (≈$6M USD)
...
|------|--------|---------------|---------------|
| **Nifty 50** | 50 stocks | ₹15,000–25,000 Cr/day | Very low; tight spreads (0.02–0.05%) |
| **Nifty Midcap 100** | 100 stocks | ₹3,000–6,000 Cr/day | Low; spreads 0.05–0.15% |
| **Nifty Smallcap 250** | 250 stocks | ₹1,000–2,500 Cr/day | Moderate; spreads 0.15–0.50% |
| **Micro-cap (<₹500 Cr mkt cap)** | Thousands | ₹10–200 Cr/day | High; spreads 0.5–3%+, impact significant |
...
| Midcap liquid | 0.10–0.25% | Meaningful for orders >₹1 Cr |
| Smallcap | 0.30–1.00% | Significant; large orders move price materially |
| Lower-circuit stocks | Undefined | Cannot trade; avoid |

**Formula:** For a ₹5 crore order in a stock with 0.25% impact cost:
```
Slippage = ₹5 Cr × 0.0025 = ₹1,25,000
```
...
- **Nifty 50 stocks:** ₹0.05–0.25 tick size; spreads 0.01–0.05% of price
- **Midcap:** ₹0.25–1.00 tick size; spreads 0.05–0.20%
- **Smallcap:** ₹1.00–5.00 tick size; spreads 0.20–1.00%
- **Liquidity degrades:** During 12:30–13:30 (lunch), spreads widen 1.5–3×; during 14:45–15:30 (pre-close), spreads narrow but volume concentrated
...
**Best practice:** Set stops at prices *within* the current circuit band. For a 5% circuit stock at ₹100, stop-loss sell must be ≥₹95 (not below). Monitor circuit proximity in real-time.
...
**BUY Side:**
| Component | Rate | Tax Base | Amount (₹100 trade) |
|-----------|------|----------|---------------------|
| Brokerage | Flat ₹20/trade OR 0.02–0.05% per side | Per trade | ₹20 flat or ₹0.05 |
| Exchange Transaction Charge | 0.00345% | On turnover | ₹0.0035 |
| SEBI Charges | 0.0001% | On turnover | ₹0.0001 |
| GST | 18% | On brokerage | ₹3.60 |
| Stamp Duty | 0.02% | On buy side turnover | ₹0.02 |
| **Total Buy Cost** | | | **≈₹23.62 + brokerage** |

**SELL Side (Intraday):**
| Component | Rate | Tax Base | Amount (₹100 trade) |
|-----------|------|----------|---------------------|
| Brokerage | Same as buy | Per trade | ₹20 flat or ₹0.05 |
| Exchange Transaction Charge | 0.00345% | On turnover | ₹0.0035 |
| STT (Securities Transaction Tax) | 0.025% | On sell side turnover | ₹0.025 |
| SEBI Charges | 0.0001% | On turnover | ₹0.0001 |
| GST | 18% | On brokerage | ₹3.60 |
| Stamp Duty | 0.03% | On sell side turnover | ₹0.03 |
| **Total Sell Cost** | | | **≈₹23.66 + brokerage + STT** |
...
def calc_nse_total_cost(trade_value, brokerage_per_side=20, brokerage_type='flat'):
    ...
    Args:
        trade_value: Single-side trade value (₹)
        brokerage_per_side: Brokerage in ₹ (flat) or % (if percentage type)
...
**Example:** ₹1,00,000 single-side trade (₹2,00,000 round trip):
```
Brokerage (flat ₹20×2):     ₹40.00
Exchange Txn (0.00345%×2):   ₹6.90
STT (0.025% sell):          ₹25.00
SEBI (0.0001%×2):            ₹0.20
GST (18%×₹40):               ₹7.20
Stamp duty (0.02%+0.03%):    ₹50.00
----------------------------------------
Total:                     ₹129.30
Effective round-trip cost: 0.065% (≈6.5 bp)
```

**For small trades (₹10,000 single-side):**
```
Effective cost ≈ 0.13–0.15% (13–15 bp) — brokerage flat fee dominates
For large trades (₹10L single-side):
...
- Example: Stock with Beta 1.2 → hedge with 1.2× notional in short futures
- **Hedge ratio:** `Hedge Contracts = (Stock Value × Beta) / (Futures Price × Lot Size)`

**Nifty lot sizes (approximate):**
| Instrument | Lot Size | Notional (Nifty ~24,500) |
|------------|----------|--------------------------|
| Nifty Futures | 25 | ₹6,12,500 |
| BankNifty Futures | 15 | ₹12,00,000+ |
...
**Formula:**
```
VIX = sqrt( (2 × discounted strike sum) / T - (forward price - strike)² / T )
```

**Key components:**
- **Option strip:** OTM puts and calls across multiple strikes (≥4 strikes above/below ATM)
- **Weighting:** Out-of-the-money options weighted by 1/K² (higher weight for strikes near ATM)
...
**VIX interpretation:**
- VIX = 15 → "Normal" market; expected daily move ≈ 15%/√252 ≈ 0.94%
...
|--------|-----------|-----------------|------------------------|---------------------|
| BULL | < 17 | > 20 | 1.5× (standard) | 2.0R |
| BULL | 17–22 | > 25 | 1.75× | 1.75R |
| HIGH_VOL | 22–28 | > 30 | 2.0× | 1.5R |
...
|ITC | 2–3% | FMCG |
|轴心 Hindustan Unilever | 2–3% | FMCG |
...
- **Passive flow:** On rebalancing date, ~₹5,000–15,000 Cr of passive buying/selling in a single day
...
```python
# For ₹1,00,000 single-side intraday trade (₹2L round trip):
ROUND_TRIP_COST_BP = 6.5  # basis points (approx., flat brokerage ₹20)
# For small trades (₹10K single-side): ~15 bp
# For large trades (₹10L single-side): ~6.5 bp
```
