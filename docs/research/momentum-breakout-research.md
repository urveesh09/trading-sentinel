...
```
MACD-ZL = EMA(close, α=2/(n+1)) - EMA(EMA(close, α=2/(n+1)), α=2/(n+1))   [Kaufman adaptation]
```
...
Keltner_ATR  = EMA20(tr) × 2
Bollinger_MA = SMA20(close)
Bollinger_Width = 2 × SMA20(std20(close))
...
pass_MC4 = close >= intraday_low + 0.80 × intraday_range
```
...
Keltner_Upper = EMA20(close) + (ATRe × K)   [ATRe = EMA14 of True Range]
Keltner_Lower = EMA20(close) - (ATRe × K)
...
```
Chandelier_Long  = HighestHigh(n) - ATR(n) × Multiplier
Chandelier_Short = LowestLow(n) + ATR(n) × Multiplier
...
```
ADR_5  = EMA14(tr) × 5   # 5-day average daily range
...
```
R_it - R_ft = α_i + β_1·(R_mt - R_ft) + β_2·SMB_t + β_3·HML_t + ε_it
```

For NSE trading:
- **Market beta (β_1):** The existing regime filter in the system (BULL vs BEAR) implicitly captures market direction. A formal β calculation would improve signal quality for high-beta stocks (>1.5) in the momentum pool.
...
Return_t+1 = α + β·Return_t   [β significantly positive across asset classes]
```
...
```
+DI  = EMA14(+DM) / ATR14 × 100
-DI  = EMA14(-DM) / ATR14 × 100
ADX  = EMA14(|+DI - -DI|) / (|+DI + -DI|) × 100
```
...
India has no direct VIX equivalent for NSE; use **India VIX** (NSE индекс India VIX) as proxy.
...
|------------|------------------------|-----------|
| VIX < 12 (calm) | 1.4× | Low volatility, less institutional activity |
| 12 ≤ VIX < 20 | 1.5× (existing) | Normal market |
| VIX > 20 (elevated) | 1.8× | High volatility, volume spikes more common; raise bar |
...
```
ATR_Pct = ATR14(close) / close × 100   [ATR as % of price]
...
```
risk_per_trade = total_capital × risk_pct_per_trade
...
```
target_vol = portfolio_value × target_risk_pct_annual
position_vol = ATR_pct_i × position_value_i
shares_i = target_vol / (ATR_pct_i × entry_i)
```
...
target_notional = momentum_pool × MOMENTUM_POOL_PCT
position_i = (raw_score_i / sum(scores)) × target_notional
```
...
# For 55% win rate, 2R avg win: Kelly = 0.55 - (0.45 / 0.55×2) = 0.55 - 0.41 = 0.14
...
| Risk-parity | `risk_pct × pool / (entry - stop)` | Current system | Medium-High |
| Volatility-targeting | `target_vol / ATR_pct × entry` | High-ATR names | Medium |
| Score-weighted | `score_i / Σscores × pool` | Conviction-weighted | Medium-High |
...
```
Chandelier_Stop_Long = Highest_High_since_entry - ATR(n) × Multiplier
...
```
atr_exit_threshold = entry_price + (ATR_14 × threshold_pct)

Threshold = 2.5: exit when price has moved 2.5× ATR above entry
Threshold = 3.0: exit at 3× ATR (aggressive)
```

**For 2R target, this maps to:** Exit at 2R ≈ 2R / ATR_pct. On a stock with ATR% = 2%, 2R = 4% move → ATR_14 × threshold must equal 4%. Threshold ≈ 2.0.
...
- 1R partial: +1R × 50% position = +0.5R
- 2R exit: +2R × 50% position = +1.0R
...
|--------|---------|-------------|-------------|
| **Chandelier** | HH - ATR×3 | Medium | Trending, smooth moves |
| **Parabolic SAR** | Prior SAR + AF×(EP - Prior SAR) | High | Strong single-direction moves |
| **Moving Average trail** | EMA20 of price | Low-Medium | Trend-following, avoids whipsaw |
| **High-water mark** | HWM - ATR×2 | Medium | Locks in incremental profit |
| **ATR % of price** | HWM - (atr_pct × HWM) | Adaptive | Volatility-normalized |
...
| MC3 volume surge | 1.5× / 1.75× (T) | 1.5–2.0× | Current lunchtime elevation is correct |
...
| Momentum risk % | 10% of pool | 7–15% | At ₹5,000 bankroll, 10% = ₹50 risk per trade |
...
- Margin interest (for MIS leveraged positions): becomes significant at >₹50,000 bankroll with 5+ positions
...
- [ ] **Volatility-targeting position sizing:** Replace fixed `MOMENTUM_RISK_PCT` shares with `target_daily_vol / (atr_pct × entry)` approach.
- [ ] **Chandelier ATR trailing stop:** Implement in `position_tracker.py` — trail stop above entry after price exceeds 1R. Use `atr_14 × 2.5` as trail distance.
...
- [ ] **Score-weighted position sizing in momentum pool:** Use `signal_score / Σ(scores)` to allocate across active momentum positions instead of equal risk.
...
# Bollinger Bands
Upper = SMA20(close) + 2 × σ20(close)
Lower = SMA20(close) - 2 × σ20(close)
...
# Keltner Channel
Keltner_MA = EMA20(close)
Keltner_Upper = EMA20(close) + 2 × ATR14

# Chandelier Stop
Chandelier_Long = HighestHigh(n) - ATR(n) × 3.0
...
shares = (portfolio_vol_target / 252) / (ATR% × entry_price)
