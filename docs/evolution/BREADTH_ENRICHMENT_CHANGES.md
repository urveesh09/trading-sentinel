...
**Status:** ✅ Ready for Stage 0 rollout. **Do not merge yet** — user reviewing.
...
- **Reward top-breadth leaders in all regimes.** Stocks in the top 20% /
  ...
  bonus, and the top quintile gets a 1.2× score multiplier that pushes
...
- **Narrow rally (e.g. 2023-style, ~30% breadth):** 5–8 fire (top-breadth
  leaders) instead of 0. Quality ↑, count ↓.
- **Crash (e.g. 2020-style, <15% breadth):** 1–2 fire (rare survivors)
  instead of blanket-reject. Quality ↑, count ↓.
...
| `python-engine/engine.py` | `evaluate_signal` signature gets `breadth_rank` + `breadth_pct_above_sma50` kwargs. R1 narrow-rally gate (L195) fires before scoring. BREADTH SCORING BONUS block (L459) applies +15/+7/-10 + 1.2× multiplier based on rank. | +45 |
...
| File | Tests |
|------|-------|
| `python-engine/tests/test_engine_breadth.py` | 12 engine integration tests: gate fires in R1 + low pct, gate exempts top-quintile, gate skips in R2/R3, gate skips when flag off, +15/+7/-10 bonus bands, 1.2× multiplier, narrow_rally_filtered in result dict. |
...
1. **`BREADTH_TIER1_PARALLELISM` default is 4, not 11** (spec §6). The
   ...
   during the scan).
2. **`BREADTH_FETCH_TIMEOUT_SECONDS=90`** added beyond spec §6's 11
...
3. **`BREADTH_DATA_DIR` added** beyond spec §6 (so 13 total settings).
...
7. **Position sizing unchanged.** OQ2 from the spec — we use the score
   multiplier (×1.2) to nudge borderline signals over the threshold,
...
- **OQ2 (position sizing for top-breadth):** the current
  implementation uses a score multiplier (×1.2) rather than
