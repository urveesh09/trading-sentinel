...
| D4 | NIFTY/SENSEX public observations run concurrently and each completed observation has its urgent lifecycle work dispatched before any optional entry/protection chain starts. A slow or failed directional chain cannot delay either index’s public observation, invalidation, target, or retirement update; explicit conditional protection has its own profile/chain branch even without an ORB entry. No partner order/holding inspection was added. | `test_partner_orchestrator.py`, `test_partner_manual_advisory.py` |
...
Result after independent-review remediation: **278 passed**. The repository currently emits one Starlette lifespan deprecation warning.
