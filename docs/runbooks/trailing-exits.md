...
3. **Chandelier trail now regime-aware:**
   - Regime 1 (calm): **3.5xATR** (was 3.0x) -- more room for mid-cap trends
   - Regime 2 (elevated): **3.0xATR** (unchanged)
   - Regime 3 (crisis): **2.5xATR** (was 3.0x) -- cut losses fast
...
3.0xATR trail**. The new behavior only applies to positions opened after this
...
A `regime_at_entry` of NULL means legacy (3.0xATR). A value of `REGIME_1_NORMAL`,
...
- **Momentum screener** -- uses its own R/R target (MOMENTUM_R_TARGET=2.0), unchanged.
  **Deliberate scope decision** (2026-06-16): momentum positions are
  ...
  The 3.5x/3.0x/2.5x trail and 5R cap are **unreachable for momentum** --
...
| 3.5x/3.0x/2.5x Chandelier trail | Catches reversals | **Never fires** -- momentum is squared at 15:15 IST, before the trail has data to act on |
...
3. **Last:** time-of-day filters. By then you'll have the data to know
   which time x regime combos are winners vs losers.
