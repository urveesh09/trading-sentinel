...
async def _claim(
    ...
    *,
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        if decision_id:
            cur = await db.execute(
                "SELECT delivered, detail FROM partner_hedge_messages WHERE kind=? AND dedup_key<>?",
                (kind, key),
            )
            for delivered, raw_detail in await cur.fetchall():
                try:
