...
` inclusive) from THIS file so it does not run again until the next update. Save the file.
</post-update-cleanup>
...
### Typography
â†’ *Consult [typography reference](reference/typography.md) for OpenType features, web font loading, and the deeper material on scales.*
...
Step 1. Read the brief once. Write down 3 concrete words for the brand voice (e.g., "warm and mechanical and opinionated", "calm and clinical and careful", "fast and dense and unimpressed", "handmade and a little weird"). NOT "modern" or "elegant" â€” those are dead categories.
...
Step 3. Browse a font catalog with the 3 brand words in mind. Sources: Google Fonts, Pangram Pangram, Future Fonts, Adobe Fonts, ABC Dinamo, Klim Type Foundry, Velvetyne. Look for something that fits the brand as a *physical object* â€” a museum exhibit caption, a hand-painted shop sign, a 1970s mainframe terminal manual, a fabric label on the inside of a coat, a children's book printed on cheap newsprint. Reject the first thing that "looks designy" â€” that's the trained reflex too. Keep looking.
...
DO NOT use overused fonts like Inter, Roboto, Arial, Open Sans, or system defaults â€” but also do not simply switch to your second-favorite. Every font in the reflex_fonts_to_reject list above is banned. Look further.
...
### Color & Theme
â†’ *Consult [color reference](reference/color-and-contrast.md) for the deeper material on contrast, accessibility, and palette construction.*
...
- Use OKLCH, not HSL. OKLCH is perceptually uniform: equal steps in lightness *look* equal, which HSL does not deliver. As you move toward white or black, REDUCE chroma â€” high chroma at extreme lightness looks garish. A light blue at 85% lightness wants ~0.08 chroma, not the 0.15 of your base color.
...
- A perp DEX consumed during fast trading sessions â†’ dark
- A hospital portal consumed by anxious patients on phones late at night â†’ light
- A children's reading app â†’ light
- A vintage motorcycle forum where users sit in their garage at 9pm â†’ dark
- An observability dashboard for SREs in a dark office â†’ dark
- A wedding planning checklist for couples on a Sunday morning â†’ light
- A music player app for headphone listening at night â†’ dark
- A food magazine homepage browsed during a coffee break â†’ light
...
DO NOT use gradient text for impact â€” see <absolute_bans> below for the strict definition. Solid colors only for text.
...
### Layout & Space
â†’ *Consult [spatial reference](reference/spatial-design.md) for the deeper material on grids, container queries, and optical adjustments.*
...
- Use a 4pt spacing scale with semantic token names (`--space-sm`, `--space-md`), not pixel-named (`--spacing-8`). Scale: 4, 8, 12, 16, 24, 32, 48, 64, 96. 8pt is too coarse â€” you'll often want 12px between two values.
- Use `gap` instead of margins for sibling spacing. It eliminates margin collapse and the cleanup hacks that come with it.
- Vary spacing for hierarchy. A heading with extra space above it reads as more important â€” make use of that. Don't apply the same padding everywhere.
...
DO NOT let body text wrap beyond ~80 characters per line. Add a max-width like 65â€“75ch so the eye can track easily.
</spatial_rules>
...
### Motion
â†’ *Consult [motion reference](reference/motion-design.md) for timing, easing, and reduced motion.*
...
### Interaction
â†’ *Consult [interaction reference](reference/interaction-design.md) for forms, focus, and loading patterns.*
...
### Responsive
â†’ *Consult [responsive reference](reference/responsive-design.md) for mobile-first, fluid design, and container queries.*
...
### UX Writing
â†’ *Consult [ux-writing reference](reference/ux-writing.md) for labels, errors, and empty states.*
